...
const arrayAccessor: BindingAccessor = (_state, parentId, index) => {
...
const stringAccessor: BindingAccessor = (state, parentId, _index, idStack, isOmitted) => {
...
const setAccessor: BindingAccessor = (state, parentId, _index, idStack, isOmitted) => {
	const args = [parentId];
...
const mapAccessor: BindingAccessor = (state, parentId, _index, idStack) => {
	const args = [parentId];
	const lastId = peek(idStack);
	if (lastId) {
		args.push(lastId);
...
const iterableFunctionLuaTupleAccessor: BindingAccessor = (state, parentId, _index, _idStack, isOmitted) => {
...
const iterableFunctionAccessor: BindingAccessor = (state, parentId, _index, _idStack, isOmitted) => {
	const callExp = luau.call(parentId);
	if (isOmitted) {
		state.prereq(
			luau.create(luau.SyntaxKind.CallStatement, {
...
const iterAccessor: BindingAccessor = (state, parentId, _index, _idStack, isOmitted) => {
