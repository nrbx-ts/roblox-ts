...
import { transformBindingName } from 'ts-transformer/nodes/binding/transformBindingName';
...
const buildIterableFunctionLuaTupleLoop: (type: ts.Type) => LoopBuilder =
	(type) => (state, statements, initializer, exp) => {
		...
		const iteratorReturnIds: luau.TemporaryIdentifier[] = [];
		...
		assert(
			luaTupleType?.aliasTypeArguments && luaTupleType.aliasTypeArguments.length === 1,
		...
		const builder = makeForLoopBuilder((_state, _initializer, exp, ids, initializers) => {
