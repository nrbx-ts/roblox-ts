...
function shouldInline(
	...
	if (ts.isMethodDeclaration(node) && node.parameters.some((parameter) => countDecorators(parameter) > 0))
		return false;
	...
	if (ts.isClassLike(node)) {
		...
		if (constructor?.parameters.some((parameter) => countDecorators(parameter) > 0)) return false;
	}
