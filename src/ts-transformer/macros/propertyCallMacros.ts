...
import type { MacroList, PropertyCallMacro } from 'ts-transformer/macros/types';
...
const STRING_CALLBACKS: MacroList<PropertyCallMacro> = {
	size: (_state, _node, expression) => luau.unary('#', expression),
...
function makeEveryOrSomeMethod(
	...
	return (state, _node, expression, args) => {
...
const READONLY_ARRAY_METHODS: MacroList<PropertyCallMacro> = {
	isEmpty: (_state, _node, expression) => luau.binary(luau.unary('#', expression), '==', luau.number(0)),
	...
	move: (_state, _node, expression, args) => {
	...
	includes: (_state, _node, expression, args) => {
	...
	indexOf: (_state, _node, expression, args) => {
	...
	map: (state, _node, expression, args) => {
		expression = state.pushToVarIfComplex(expression, 'exp');
	...
	mapFiltered: (state, _node, expression, args) => {
		expression = state.pushToVarIfComplex(expression, 'exp');
	...
	filterUndefined: (state, _node, expression) => {
		expression = state.pushToVarIfComplex(expression, 'exp');
	...
	filter: (state, _node, expression, args) => {
		expression = state.pushToVarIfComplex(expression, 'exp');
	...
	reduce: (state, _node, expression, args) => {
		expression = state.pushToVarIfComplex(expression, 'exp');
	...
	find: (state, _node, expression, args) => {
		expression = state.pushToVarIfComplex(expression, 'exp');
	...
	findIndex: (state, _node, expression, args) => {
		expression = state.pushToVarIfComplex(expression, 'exp');
...
const ARRAY_METHODS: MacroList<PropertyCallMacro> = {
	...
	shift: (_state, _node, expression) => luau.call(luau.globals.table.remove, [expression, luau.number(1)]),
	...
	insert: (_state, _node, expression, args) => {
		return luau.call(luau.globals.table.insert, [expression, offset(args[0], 1), args[1]]);
	},

	remove: (_state, _node, expression, args) => luau.call(luau.globals.table.remove, [expression, offset(args[0], 1)]),
...
const READONLY_SET_MAP_SHARED_METHODS: MacroList<PropertyCallMacro> = {
	isEmpty: (_state, _node, expression) => luau.binary(luau.call(luau.globals.next, [expression]), '==', luau.nil()),

	size: (state, _node, expression) => {
	...
	has: (_state, _node, expression, args) => {
...
const READONLY_MAP_METHODS: MacroList<PropertyCallMacro> = {
	...
	get: (_state, _node, expression, args) =>
		luau.create(luau.SyntaxKind.ComputedIndexExpression, {
			expression: convertToIndexableExpression(expression),
			index: args[0],
		}),
