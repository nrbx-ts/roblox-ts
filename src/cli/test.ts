...
function addCompileTests(data: ProjectData, diagnosticsFolder: string) {
...
describe('should compile tests project', () => {
	...
	addCompileTests(data, diagnosticsFolder);
});

describe('should compile react tests project', () => {
	const data = createProjectData(
		path.join(PACKAGE_ROOT, 'tests', 'tsconfig.react.json'),
		Object.assign({}, DEFAULT_PROJECT_OPTIONS, {
			project: '',
			allowCommentDirectives: true,
			optimizedLoops: true,
	...
	const diagnosticsFolder = path.join(PACKAGE_ROOT, 'tests', 'src-react', 'diagnostics');
	addCompileTests(data, diagnosticsFolder);
});
